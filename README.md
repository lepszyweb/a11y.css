...
Możesz użyć głównego pliku **a11y.css** albo jego wariacji jako skryptozakładki. Na polskiej stronie projektu w bocznym pasku umieściliśmy kilka przycisków, za pomocą których możesz dodać do swoich zakładek w przeglądarce. Gdy wejdziesz na stronę, którą chcesz zbadać, wystarczy wywołać łącze zakładki. Gdy skończysz badanie, po prostu odśwież stronę. Skryptozakładka niczego na Twojej stronie nie zmieni, spowoduje jedynie, że od momentu jej uaktywnienia do odświeżenia strony, przeglądarka zastosuje do odtworzenia strony dodatkowy arkusz stylów, który spowoduje wyświetlenie informacji o występujących w kodzie HTML strony błędach i niedostatkach oraz przypadkach, które mogą powodować problemy.   
